...
package mylib::myPlot;

$mylib::myPlot::VERSION = '0.11'; 
...
sub _drawData {
  ...
  foreach $dataSetLabel (keys %{$self->{'_data'}}) {
    ...
    $self->{'_im'}->arc($px, $py,6,6,0,360,$self->{$color})
    ...
    # debugging
    if ($self->{'_debugging'}) {
      $self->{'_im'}->string(gdGiantFont,$px,$py-10,
			     "0($px,$py)",$self->{$color});
	...
	$self->{'_im'}->line($prevpx, $prevpy, $px, $py, $self->{$color});
      ...
      # debugging
      if ($self->{'_debugging'}) {
	$self->{'_im'}->string(gdGiantFont,$px-10,$py+10,
			       "$i($px,$py)",$self->{$color});
...
sub _drawAxes {
  ...
  my $self = shift;
  my ($w,$h) = (gdGiantFont->width, gdGiantFont->height);
  ...
  ### vertical axis
  my ($p3x, $p3y) = $self->_data2pxl (0, $self->{'_ymin'});
  my ($p4x, $p4y) = $self->_data2pxl (0, $self->{'_ymax'});
  $self->{'_im'}->line($p3x, $p3y, $p4x, $p4y, $self->{'_black'});
  $self->{'_im'}->rectangle($p2x, $p2y, $p4x, $p4y, $self->{'_black'});
  ### axis label
  $xStart = $p4x - length ($self->{'_vertAxisLabel'}) * $w / 2 - 80;
  $self->{'_im'}->stringUp (gdGiantFont, ($xStart>10 ? $xStart : 10), 5.5*$p4y,# - 5*$h,
			  $self->{'_vertAxisLabel'},
			  $self->{'_black'});
  
  print STDERR "Ver: p1 ($p3x, $p3y) p2 ($p4x, $p4y)\n"
    if $self->{'_debugging'};
...
sub _drawTitle {
  ...
  # centered above chart (mrama)
  my ($px,$py) = ($self->{'_imx'}/2, # $self->{'_vertGraphOffset'}/2);
		  $self->{'_imy'} - $self->{'_vertGraphOffset'}*7.7);

  ($px,$py) = ($px - length ($self->{'_title'}) * $w/2, $py+$h/2);
  $self->{'_im'}->string (gdGiantFont, $px, $py,
			  $self->{'_title'},
			  $self->{'_black'}); 
}
